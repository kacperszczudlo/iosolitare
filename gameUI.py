...
class GameUI:
    ...
    def create_button(self, text, x, y, width, command=None):
        button = Button(self.gameSetup.window, text=text, font=("Arial", 12, "bold"), fg="white", bd=0,
                        highlightthickness=0,
                        bg="#5C4033", state="normal", command=command)
        button.place(x=x, y=y, width=width, height=31)

    def create_label(self, text, x, y, width):
        label = Label(self.gameSetup.window, text=text, font=("Arial", 12, "bold"), fg="white", bd=0,
                      highlightthickness=0,
                      bg="#5C4033")
    ...
    def display_stock_pile(self, wyjebane=[]):
        ...
        for i in range(len(self.gameSetup.first_deal.columns) * (len(self.gameSetup.first_deal.columns) + 1) // 2,
                       len(self.gameSetup.deck.cards)):
    ...
    def start_timer(self):
        self.elapsed_time = 0
        self.timer_label = Label(self.gameSetup.window, text="Czas: 00:00", font=("Arial", 12, "bold"), fg="white",
                                 bg="#5C4033")
    ...
    def update_move_counter(self, count):
        self.move_counter_label = Label(self.gameSetup.window, text=f"Ruchy: {count}", font=("Arial", 12, "bold"),
                                        fg="white", bg="#5C4033")
        self.move_counter_label.place(x=846, y=16, width=119, height=31)

    def init_score(self):
        self.score = 0
        self.score_label = Label(self.gameSetup.window, text=f"Punkty: 0", font=("Arial", 12, "bold"), fg="white",
                                 bg="#5C4033")
        self.score_label.place(x=711, y=16, width=119, height=31)

    def update_score(self, score):
        if self.score_label is None:
            self.score_label = Label(self.gameSetup.window, text=f"Punkty: {self.score}", font=("Arial", 12, "bold"),
                                     fg="white",
                                     bg="#5C4033")
            self.score_label.place(x=711, y=16, width=119, height=31)
        self.score += score
